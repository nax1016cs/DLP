...
class record:
    ...
    def plot(self):
        ...
        plt.plot(epoch, self.bleu_score, 'ro', label = "BLEU4-score", linewidth=0.5 )
        plt.plot(epoch, self.gaussian_score, 'bo',  label = "gaussian_score", linewidth=0.5  )
        plt.plot(epoch, self.kld, label = "KLD" , linewidth = 1 )
        plt.plot(epoch, self.cross_entropy, label = "CrossEntropy" , linewidth = 1 )
        plt.plot(epoch, self.kld_weight, label = "kld_weight" , linewidth=1, linestyle="dashed")
        plt.plot(epoch, self.teacher_forcing, label = "teacher ratio" , linewidth=1, linestyle="dashed")

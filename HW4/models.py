...
class CVAE(nn.Module):
    ...
    def kld_weight(self, iteration, method):
        if method == "monotonic":
            slope = 1 / 20000
            return min(1, (iteration % (40000)) * slope)
        else:
            pass
    ...
    def train(self, train_dataset, optimizer, epoch):
        ...
        for i in range(len(train_dataset)):
            ...
            loss = cross_entropy_loss + kld_loss * self.kld_weight(epoch * len(train_dataset) + i, 'monotonic')
            loss.backward()
            total_cross_entropy_loss += cross_entropy_loss.item()
            total_kld_loss += kld_loss.item()
            optimizer.step()
        return total_cross_entropy_loss / len(train_dataset), total_kld_loss / len(train_dataset), self.kld_weight(i, 'monotonic'), teacher_forcing_ratio
